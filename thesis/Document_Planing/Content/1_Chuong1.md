...
Sự bùng nổ của các mô hình ngôn ngữ lớn đánh dấu sự chuyển mình từ so khớp mẫu sang hiểu và sinh ngôn ngữ. Bài toán xử lý hội thoại chuyển sang một giai đoạn mới nhờ khả năng phân tích ngữ cảnh dài, suy luận logic cơ bản và sinh ngôn ngữ tự nhiên như con người.
...
- Kiến trúc sinh văn bản tự động tạo phản hồi theo xác suất phân phối từ của mô hình ngôn ngữ, cho phép hội thoại cực kỳ tự nhiên. Kiến trúc này tiềm ẩn rủi ro hiện tượng ảo giác, tức là sinh ra thông tin có vẻ hợp lý nhưng hoàn toàn sai sự thật do không có cơ chế đối chiếu.
- Kiến trúc lai kết hợp hai cơ chế này, sử dụng thông tin truy xuất chính xác từ cơ sở dữ liệu nội bộ làm ngữ cảnh nền tảng, sau đó cấp cho mô hình ngôn ngữ để sinh phản hồi tự nhiên và chuẩn xác dựa trên ngữ cảnh đó.
...
## 1.2. Mô hình ngôn ngữ lớn và kiến trúc Transformer

### 1.2.1. Kiến trúc Transformer và cơ chế tự chú ý
Các mô hình ngôn ngữ hiện đại như GPT-4, Llama 3 hay Google Gemini đều được xây dựng dựa trên kiến trúc Transformer, lần đầu được giới thiệu bởi Vaswani và cộng sự (2017). Khác biệt cốt lõi của Transformer so với các mạng nơ-ron tuần tự truyền thống nằm ở cơ chế tự chú ý.
...
### 1.2.2. Vòng đời huấn luyện của mô hình ngôn ngữ
Để trở thành một trợ lý ảo thực thụ, một mô hình ngôn ngữ phải trải qua ba giai đoạn huấn luyện cốt lõi:
...
### 1.2.3. Hạn chế của mô hình ngôn ngữ trong hệ thống tri thức đóng
Khi ứng dụng mô hình ngôn ngữ nguyên bản vào hệ thống tuyển sinh, bài toán đối mặt với ba rào cản kỹ thuật lớn:
- Hiện tượng ảo giác là rủi ro nguy hiểm nhất. Bản chất của mô hình ngôn ngữ là thống kê xác suất, nó không có khái niệm biết hay không biết. Khi được hỏi về điểm chuẩn ngành Công nghệ thông tin của trường năm nay, nếu chưa từng thấy dữ liệu này, nó sẽ sinh ra một con số ngẫu nhiên vì cấu trúc đó hợp lý về mặt ngữ pháp, dù thông tin đó là hoàn toàn bịa đặt.
...
Kỹ thuật RAG được công bố lần đầu bởi Patrick Lewis và cộng sự (2020) là giải pháp triệt để nhằm khắc phục hiện tượng ảo giác và hạn chế về dữ liệu tĩnh của mô hình ngôn ngữ. Thay vì buộc mô hình phải ghi nhớ mọi thứ vào các trọng số mạng nơ-ron, RAG tách bạch tri thức ra một cơ sở dữ liệu độc lập.

Kiến trúc tổng quát vận hành qua hai pha chính: pha lập chỉ mục và pha truy xuất sinh văn bản.
- Pha lập chỉ mục diễn ra độc lập, trong đó tài liệu văn bản thô được làm sạch, cắt thành nhiều mảnh nhỏ, sau đó đưa qua một mô hình nhúng để biến đổi thành các vector đa chiều và lưu trữ tại cơ sở dữ liệu vector. Quá trình này có thể thực hiện định kỳ mỗi khi có tài liệu mới.
- Pha truy xuất khởi động khi câu hỏi người dùng được mô hình nhúng chuyển đổi thành vector. Cơ sở dữ liệu sử dụng thuật toán tìm kiếm khoảng cách gần nhất để trích xuất ra các mảnh tài liệu liên quan nhất. Các mảnh tài liệu này được đẩy vào ngữ cảnh cùng với câu hỏi gốc, ép mô hình ngôn ngữ đóng vai trò đọc hiểu để rút trích câu trả lời từ chính đoạn văn bản vừa tìm được.
...
- Phân rã theo kích thước cố định tiến hành cắt tài liệu thành các khối có số ký tự bằng nhau. Phương pháp này nhanh, dễ triển khai nhưng làm gãy đôi các câu hoặc đoạn văn, phá vỡ cấu trúc ngữ nghĩa ngữ pháp.
...
Bảng dưới đây so sánh RAG với các phương pháp tối ưu mô hình ngôn ngữ phổ biến:
...
*Bảng 1.1: So sánh các phương pháp điều khiển mô hình ngôn ngữ*
...
Với một cơ sở dữ liệu có hàng triệu mảnh văn bản, việc tính toán khoảng cách từ câu hỏi đến toàn bộ các mảnh văn bản sẽ gây nghẽn cổ chai hệ thống. Các cơ sở dữ liệu vector hiện đại giải quyết bài toán này bằng thuật toán tìm kiếm lân cận gần đúng phân cấp mạng thế giới nhỏ.
...
*Bảng 1.2: So sánh thông số hệ quản trị cơ sở dữ liệu vector*
...
- Máy chủ nền tảng phát triển bằng ngôn ngữ Python kết hợp FastAPI. Cấu trúc xử lý bất đồng bộ đáp ứng hàng trăm luồng truy vấn mạng song song. Giao thức dòng sự kiện liên tục được ứng dụng để truyền tải tức thời các mảnh văn bản thẳng về thiết bị người dùng thay vì chờ sinh xong toàn bộ câu, giúp cảm giác độ trễ giảm xuống dưới 1 giây.
...
- Giao diện người dùng tích hợp trực tiếp lên nền tảng nhắn tin Zalo Bot giúp tiếp cận thí sinh mà không cần cài đặt thêm ứng dụng di động. Phân hệ bảng điều khiển quản trị được xây dựng dựa trên ngăn xếp web hiện đại React kết nối thông qua giao thức API.

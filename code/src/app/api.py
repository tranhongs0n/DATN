    ...
    try:
        ...
        if event_name == "user_send_text":
            ...
            if user_id and text:
                ...
                full_response = ""
                # Provide empty history for now (long-term memory can be added later)
                for chunk in chat_response(text, []):
                    full_response += chunk
                
                # 2. Send back to Zalo
                logger.info(f"Sending response to Zalo user {user_id}: {full_response[:100]}...")
                zalo_api.send_text_message(user_id, full_response)
...
async def rebuild_index():
    logger.info("Triggered rebuild_index from Admin UI")

...
class IndexingService:
    ...
    def get_file_status_list(self):
        ...
        # Add files that are in database but missing from the local filesystem
    ...
    def save_uploads(self, files):
        """Saves uploaded files to the AdminUploads directory."""
        if not files:
            return 0, None
        ...
        count = 0
        for f in files:
            dest = target_dir / os.path.basename(f.name)
    ...
    def convert_unsupported_file(self, filename: str, engine: 'MultimodalEngine'):
        ...
        try:
            ...
            # Simple markdown to docx logic (splitting by lines)
